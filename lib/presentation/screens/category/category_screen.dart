...
class AppCategory {
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'name': name, 'icon': icon, 'type': type.name};
...
class CategoryNotifier extends StateNotifier<List<AppCategory>> {
  ...
  Future<void> _load() async {
    ...
    if (raw != null && raw.isNotEmpty) {
      try {
        ...
        state = list.map((e) => AppCategory.fromJson(e as Map<String, dynamic>)).toList();
        return;
      } catch (_) {}
    }
    state = _defaults();
  ...
  Future<void> add(AppCategory cat) async    { state = [...state, cat]; await _persist(); }
  Future<void> remove(String id) async       { state = state.where((c) => c.id != id).toList(); await _persist(); }
  ...
  Future<void> restoreAll(List<AppCategory> cats) async { state = cats; await _persist(); }
}
...
class _CategoryScreenState extends ConsumerState<CategoryScreen>
  ...
  void _showAdd(TransactionType type) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx2, setS) => _sheet(
        title: type == TransactionType.income ? '↑ নতুন আয়ের Category' : '↓ নতুন খরচের Category',
        ctrl: ctrl, selIcon: sel, type: type,
        onIconTap: (ic) => setS(() => sel = ic),
        onSave: () {
          if (ctrl.text.trim().isEmpty) return;
          ref.read(categoriesProvider.notifier).add(AppCategory(
            id: DateTime.now().millisecondsSinceEpoch.toString(),
            name: ctrl.text.trim(), icon: sel, type: type));
          Navigator.pop(ctx2);
          _snack('✅ "${ctrl.text.trim()}" যোগ হয়েছে!', AppColors.teal);
        },
  ...
  void _showEdit(AppCategory cat) {
    ...
    showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (ctx) => StatefulBuilder(builder: (ctx2, setS) => _editSheet(
        cat: cat, ctrl: ctrl, selIcon: sel,
        onIconTap: (ic) => setS(() => sel = ic),
        onSave: () {
          if (ctrl.text.trim().isEmpty) return;
          ref.read(categoriesProvider.notifier).edit(cat.id, ctrl.text.trim(), sel);
          Navigator.pop(ctx2);
          _snack('✅ Category update হয়েছে!', AppColors.gold);
        },
        onDelete: () { Navigator.pop(ctx2); _confirmDelete(cat); },
        onCancel: () => Navigator.pop(ctx2),
      )));
  }
  ...
  void _confirmDelete(AppCategory cat) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: Row(children: [
        Text(cat.icon, style: const TextStyle(fontSize: 22)), const SizedBox(width: 10),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            _snack('"${cat.name}" delete হয়েছে', AppColors.rose);
          },
  ...
  Widget _iconGrid(String sel, Color borderColor, Function(String) onTap) => SizedBox(
    height: 160,
    child: GridView.builder(
      ...
      itemBuilder: (_, i) => GestureDetector(
        onTap: () => onTap(_icons[i]),
        child: AnimatedContainer(duration: const Duration(milliseconds: 150),
          decoration: BoxDecoration(
            color: sel == _icons[i] ? borderColor.withOpacity(0.2) : Colors.white.withOpacity(0.06),
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: sel == _icons[i] ? borderColor : Colors.transparent, width: 1.5)),
          child: Center(child: Text(_icons[i], style: const TextStyle(fontSize: 18)))))));

  Widget _sheet({
    required String title, required TextEditingController ctrl, required String selIcon,
    required TransactionType type, required Function(String) onIconTap,
    required VoidCallback onSave, required VoidCallback onCancel,
  }) {
    final color = type == TransactionType.income ? AppColors.teal : AppColors.rose;
    final grad  = type == TransactionType.income ? AppColors.gradTeal : AppColors.gradRose;
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(title, style: const TextStyle(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w800, fontFamily: 'Syne')),
        const SizedBox(height: 20),
        TextField(controller: ctrl, autofocus: true, style: const TextStyle(color: AppColors.textPrimary, fontSize: 15),
          decoration: InputDecoration(hintText: 'Category নাম লেখো', hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 14),
        ...
        _iconGrid(selIcon, color, onIconTap),
        const SizedBox(height: 20),
  ...
  Widget _editSheet({
    required AppCategory cat, required TextEditingController ctrl, required String selIcon,
    required Function(String) onIconTap,
    required VoidCallback onSave, required VoidCallback onDelete, required VoidCallback onCancel,
  }) {
    return Container(
      padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
      decoration: const BoxDecoration(color: Color(0xFF141C2E), borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 20),
        Row(children: [
          Text(selIcon, style: const TextStyle(fontSize: 24)), const SizedBox(width: 10),
          const Text('Category Edit করো', style: TextStyle(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w800, fontFamily: 'Syne')),
        ]),
        const SizedBox(height: 20),
        TextField(controller: ctrl, autofocus: true, style: const TextStyle(color: AppColors.textPrimary, fontSize: 15),
          decoration: InputDecoration(hintText: 'নতুন নাম লেখো', hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 14),
            filled: true, fillColor: Colors.white.withOpacity(0.06),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none),
        ...
        const SizedBox(height: 10),
        _iconGrid(selIcon, AppColors.gold, onIconTap),
        const SizedBox(height: 20),
        Row(children: [
          Expanded(child: GestureDetector(onTap: onSave,
            child: Container(height: 50, decoration: BoxDecoration(gradient: AppColors.gradGold, borderRadius: BorderRadius.circular(14)),
              alignment: Alignment.center, child: const Text('সেভ করো', style: TextStyle(color: Color(0xFF0A0E1A), fontSize: 15, fontWeight: FontWeight.w800, fontFamily: 'Syne'))))),
          const SizedBox(width: 10),
          GestureDetector(onTap: onDelete,
            child: Container(height: 50, width: 50,
              decoration: BoxDecoration(color: AppColors.rose.withOpacity(0.15), borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.rose.withOpacity(0.4))),
      ...
      children: [
        Padding(padding: const EdgeInsets.only(bottom: 8),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Icon(Icons.info_outline_rounded, color: AppColors.textDim, size: 12), const SizedBox(width: 4),
            const Text('ট্যাপ = বিস্তারিত · ধরে রাখো = Edit', style: TextStyle(color: AppColors.textDim, fontSize: 11)),
          ])),
        ...cats.map((cat) {
          ...
          return GestureDetector(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => _CatDetailScreen(category: cat, color: color))),
            // ── Long press → Edit ──────────────────────────────────
            ...
            child: Container(
              ...
              child: Row(children: [
                Container(width: 50, height: 50,
                  decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(15), border: Border.all(color: color.withOpacity(0.25))),
...
class _CatDetailScreen extends ConsumerWidget {
  final AppCategory category;
  final Color color;
  const _CatDetailScreen({required this.category, required this.color});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final liveTxs = ref.watch(transactionsProvider).maybeWhen(
      data: (txs) => txs.where((t) => t.category == category.name).toList()
      ...
      body: Column(children: [
          ...
          child: Container(padding: const EdgeInsets.all(20),
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('৳${total.toStringAsFixed(0)}',
                    style: TextStyle(color: color, fontSize: 28, fontWeight: FontWeight.w800, fontFamily: 'Syne')),
        ...
        liveTxs.isEmpty
            ? Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('এই Category তে কোনো লেনদেন নেই', style: const TextStyle(color: AppColors.textMuted, fontSize: 14)),
                Text('Add ➕ থেকে "${category.name}" বেছে যোগ করো',
                    style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.w600), textAlign: TextAlign.center),
              ])))
                  ...
                  return Dismissible(
                    ...
                    confirmDismiss: (_) async => await showDialog<bool>(context: context,
                      builder: (dCtx) => AlertDialog(
                        ...
                        actions: [
                          TextButton(onPressed: () => Navigator.pop(dCtx, false), child: const Text('না', style: TextStyle(color: AppColors.textMuted, fontSize: 15))),
