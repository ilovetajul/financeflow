...
class AppCategory {
  ...
  AppCategory copyWith({String? name, String? icon}) =>
      AppCategory(id: id, name: name ?? this.name, icon: icon ?? this.icon, type: type);
}
...
class CategoryNotifier extends StateNotifier<List<AppCategory>> {
  ...
  void edit(String id, String newName, String newIcon) {
    state = state.map((c) => c.id == id ? c.copyWith(name: newName, icon: newIcon) : c).toList();
  }
}
...
class _CategoryScreenState extends ConsumerState<CategoryScreen> with SingleTickerProviderStateMixin {
  ...
  void _showAddSheet(TransactionType type) {
          ...
          child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            TextField(controller: nameCtrl, autofocus: true,
              ...
              decoration: InputDecoration(
                hintText: 'Category নাম লেখো', hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 14),
            ...
            SizedBox(height: 160, child: GridView.builder(
              ...
              itemCount: _catIcons.length,
              itemBuilder: (_, i) => GestureDetector(
                onTap: () => setS(() => selectedIcon = _catIcons[i]),
                child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                  decoration: BoxDecoration(
                    color: selectedIcon == _catIcons[i] ? (type == TransactionType.income ? AppColors.teal.withOpacity(0.2) : AppColors.rose.withOpacity(0.2)) : Colors.white.withOpacity(0.06),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: selectedIcon == _catIcons[i] ? (type == TransactionType.income ? AppColors.teal : AppColors.rose) : Colors.transparent, width: 1.5)),
                  child: Center(child: Text(_catIcons[i], style: const TextStyle(fontSize: 18))))))),
            const SizedBox(height: 20),
            GestureDetector(
              onTap: () {
                ...
                ref.read(categoriesProvider.notifier).add(AppCategory(
                  ...
                  name: nameCtrl.text.trim(), icon: selectedIcon, type: type));
                Navigator.pop(ctx2);
  ...
  void _showEditSheet(AppCategory cat) {
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx2, setS) {
        return Container(
          ...
          child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 20),
            Row(children: [Text(selectedIcon, style: const TextStyle(fontSize: 24)), const SizedBox(width: 10),
            ...
            TextField(controller: nameCtrl, autofocus: true,
              style: const TextStyle(color: AppColors.textPrimary, fontSize: 15),
              decoration: InputDecoration(
                hintText: 'নতুন নাম লেখো', hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 14),
                filled: true, fillColor: Colors.white.withOpacity(0.06),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none),
            ...
            SizedBox(height: 160, child: GridView.builder(
              ...
              itemBuilder: (_, i) => GestureDetector(
                ...
                child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                  decoration: BoxDecoration(
                    color: selectedIcon == _catIcons[i] ? AppColors.gold.withOpacity(0.2) : Colors.white.withOpacity(0.06),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: selectedIcon == _catIcons[i] ? AppColors.gold : Colors.transparent, width: 1.5)),
                  child: Center(child: Text(_catIcons[i], style: const TextStyle(fontSize: 18))))))),
            const SizedBox(height: 20),
            Row(children: [
              Expanded(child: GestureDetector(
                onTap: () {
                  if (nameCtrl.text.trim().isEmpty) return;
                  ref.read(categoriesProvider.notifier).edit(cat.id, nameCtrl.text.trim(), selectedIcon);
                  Navigator.pop(ctx2);
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    backgroundColor: AppColors.gold, behavior: SnackBarBehavior.floating,
                    content: Text('✅ Category update হয়েছে!', style: TextStyle(color: Color(0xFF0A0E1A), fontWeight: FontWeight.w700))));
                },
                child: Container(height: 50, decoration: BoxDecoration(gradient: AppColors.gradGold, borderRadius: BorderRadius.circular(14)),
                  alignment: Alignment.center,
                  child: const Text('সেভ করো', style: TextStyle(color: Color(0xFF0A0E1A), fontSize: 15, fontWeight: FontWeight.w800, fontFamily: 'Syne'))))),
              ...
              GestureDetector(
                ...
                child: Container(height: 50, width: 50,
                  decoration: BoxDecoration(color: AppColors.rose.withOpacity(0.15), borderRadius: BorderRadius.circular(14),
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                ...
                content: Text('"${cat.name}" delete হয়েছে', style: const TextStyle(color: Colors.white))));
            },
      ...
      body: SafeArea(child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 0),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            GestureDetector(
              onTap: () => _showAddSheet(isIncome ? TransactionType.income : TransactionType.expense),
              child: Container(
        ...
        Expanded(child: TabBarView(controller: _tab, children: [
          _CatList(categories: incCats, allTxs: allTxs, color: AppColors.teal,
            onAdd: () => _showAddSheet(TransactionType.income), onEdit: _showEditSheet),
          _CatList(categories: expCats, allTxs: allTxs, color: AppColors.rose,
            onAdd: () => _showAddSheet(TransactionType.expense), onEdit: _showEditSheet),
        ])),
      ])),
    );
  }
...
class _CatList extends StatelessWidget {
  ...
  const _CatList({required this.categories, required this.allTxs, required this.color, required this.onAdd, required this.onEdit});
      ...
      Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(14),
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('৳${totalAmount.toStringAsFixed(0)}', style: TextStyle(color: color, fontSize: 20, fontWeight: FontWeight.w800, fontFamily: 'Syne')),
          ]),
          const Text('ধরে রাখো = Edit', style: TextStyle(color: AppColors.textDim, fontSize: 10)),
        ])),
      ...categories.map((cat) {
        final catTxs = allTxs.where((t) => t.category == cat.name).toList();
        ...
        return GestureDetector(
          ...
          onTap: () => Navigator.push(context, MaterialPageRoute(
            builder: (_) => _CatDetail(category: cat, color: color))),
...
class _CatDetail extends ConsumerWidget {
  final AppCategory category;
  final Color color;
  const _CatDetail({required this.category, required this.color});
          ...
          child: Container(padding: const EdgeInsets.all(20),
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('৳${total.toStringAsFixed(0)}', style: TextStyle(color: color, fontSize: 28, fontWeight: FontWeight.w800, fontFamily: 'Syne')),
