  ...
  void _openCalculator() {
    showModalBottomSheet(
      ...
      builder: (_) => _CalculatorSheet(
        onResult: (result) => setState(() => _amountCtrl.text = result),
      ),
    );
  }
              ...
              child: Row(children: [TransactionType.income, TransactionType.expense].map((t) {
                ...
                return Expanded(child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Text(isInc ? '↑  Income' : '↓  Expense',
            ...
            cats.isEmpty
                ? Container(padding: const EdgeInsets.all(20),
                    ...
                    child: Center(child: Column(children: [
                      ...
                      const SizedBox(height: 8),
                      Text('নিচের 🏷️ Category ট্যাব থেকে যোগ করো', textAlign: TextAlign.center,
                        style: TextStyle(color: _col, fontSize: 13, fontWeight: FontWeight.w600)),
                    ])))
                : GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: sel ? _col.withOpacity(0.15) : Colors.white.withOpacity(0.04),
                            border: Border.all(color: sel ? _col : Colors.white.withOpacity(0.07), width: 1.5),
...
// ══════════════════════════════════════════════════════════════════
// CALCULATOR — Fixed: result is computed inline, no async setState race
// ══════════════════════════════════════════════════════════════════
class _CalculatorSheet extends StatefulWidget {
  final Function(String) onResult;
  const _CalculatorSheet({required this.onResult});
  @override
  State<_CalculatorSheet> createState() => _CalcState();
}

class _CalcState extends State<_CalculatorSheet> {
  String _display  = '0';
  String _expr     = '';
  double? _first;
  String? _op;
  bool _fresh = false; // next digit starts fresh

  // ── Compute result without relying on setState ────────────────
  String _compute() {
    if (_first == null || _op == null) return _display;
    ...
    switch (_op) {
      ...
      case '-': res = _first! - second; break;
      case '×': res = _first! * second; break;
      case '÷': res = second != 0 ? _first! / second : 0; break;
    }
    if (res == res.truncateToDouble()) return res.toInt().toString();
    return double.parse(res.toStringAsFixed(4)).toString();
  }

  void _press(String v) {
    setState(() {
      if (v == 'C') {
        _display = '0'; _expr = ''; _first = null; _op = null; _fresh = false;
      } else if (v == '⌫') {
        _display = _display.length > 1 ? _display.substring(0, _display.length - 1) : '0';
      } else if (['+', '-', '×', '÷'].contains(v)) {
        _first = double.tryParse(_display);
        _op = v; _expr = '$_display $v'; _fresh = true;
      } else if (v == '=') {
        if (_first != null && _op != null) {
          final result = _compute();
          _expr = '$_expr $_display =';
          _display = result;
          _first = null; _op = null; _fresh = true;
        }
      } else if (v == '.') {
        if (_fresh) { _display = '0.'; _fresh = false; }
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 4),
          child: Align(alignment: Alignment.centerRight,
            child: Text(_expr, style: const TextStyle(color: AppColors.textMuted, fontSize: 13)))),
        // Display
        ...
        Padding(
          ...
          child: Column(children: [
            ...
            Row(children: [
              _btn('0', flex: 2),
              _btn('.'),
              // ✓ Use: compute result inline then pass to parent
              Expanded(child: Padding(padding: const EdgeInsets.all(4),
                child: GestureDetector(
                  onTap: () {
                    final result = _compute();   // compute without setState
                    widget.onResult(result);      // send to amount field
