...
// শুধু PIN dialog অংশটা replace করো
// ═══════════════════════════════════════════════════════════

// _showSetPinDialog() method টা এভাবে replace করো:

/*
  void _showSetPinDialog() {
    int step = 1;
    String pin1 = '';
    // প্রতিটা step এ আলাদা controller ব্যবহার করো
    ...
    showDialog(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setS) {
          final currentCtrl = step == 1 ? ctrl1 : ctrl2;
          // step বদলালে input clear হয়
          return AlertDialog(...);
        }
      )
    );
...
// ── সহজ সমাধান: পুরো profile_screen.dart replace করো ──
// নিচেরটা হলো fixed version
...
class UserProfile {
  ...
  const UserProfile({
    ...
    this.photoPath, this.pinEnabled = false});
...
class _AppLockScreenState
  ...
  Future<void> _checkLock() async {
    final enabled =
        await ref.read(profileProvider.notifier).isPinEnabled();
  ...
  Widget build(BuildContext context) {
    ...
    if (_locked) return PinEntryScreen(
...
class _PinEntryScreenState
    extends ConsumerState<PinEntryScreen> {
  String _pin = '';
  bool _error = false;
  int _attempts = 0;
      ...
      body: SafeArea(
        child: Column(mainAxisAlignment: MainAxisAlignment.center,
          ...
          Text(profile.name.isNotEmpty
              ? 'স্বাগতম, ${profile.name}!' : 'FinanceFlow',
            style: const TextStyle(color: AppColors.textPrimary,
                fontSize: 20, fontWeight: FontWeight.w800,
                fontFamily: 'Syne')),
          const SizedBox(height: 8),
          Text(_error ? '❌ ভুল PIN! আবার চেষ্টা করো'
  ...
  Widget _Row(List<String> d) => Row(
    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
    children: d.map((x) => _Btn(x, () => _add(x))).toList());
...
// ═══════════════════════════════════════════════════════════
// PROFILE SCREEN — PIN setup FIXED
// ═══════════════════════════════════════════════════════════
...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(backgroundColor: AppColors.teal,
  ...
  // Step 1: নতুন PIN দাও
  void _showSetPinStep1() {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => _PinDialog(
        title: '🔐 নতুন PIN দাও',
        subtitle: '৪ সংখ্যার PIN লেখো',
        onComplete: (pin1) {
          Navigator.pop(context);
          // Step 1 শেষ হলে Step 2 দেখাও
          Future.delayed(const Duration(milliseconds: 200), () {
            _showSetPinStep2(pin1);
          });
        },
        onCancel: () => Navigator.pop(context)));
  }

  // Step 2: PIN confirm করো — নতুন dialog, নতুন empty input
  void _showSetPinStep2(String pin1) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => _PinDialog(
        title: '🔐 PIN নিশ্চিত করো',
        subtitle: 'আবার একই PIN দাও',
        onComplete: (pin2) {
          Navigator.pop(context);
          if (pin1 == pin2) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(backgroundColor: AppColors.teal,
                behavior: SnackBarBehavior.floating,
                content: Text('✅ PIN চালু হয়েছে! অ্যাপ এখন লক থাকবে।',
                  style: TextStyle(color: Colors.white,
                      fontWeight: FontWeight.w600))));
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(backgroundColor: AppColors.rose,
                behavior: SnackBarBehavior.floating,
                content: Text('❌ PIN মিলেনি! আবার চেষ্টা করো',
                  style: TextStyle(color: Colors.white))));
            // মিলেনি → আবার শুরু থেকে
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          // Name
          const Text('তোমার নাম', style: TextStyle(
              color: AppColors.textMuted, fontSize: 12,
              fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          ...
          // Phone
          const Text('মোবাইল নম্বর', style: TextStyle(
              color: AppColors.textMuted, fontSize: 12,
              fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          ...
          // Security
          const Text('নিরাপত্তা', style: TextStyle(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('PIN Lock', style: TextStyle(
                        color: AppColors.textPrimary, fontSize: 14,
                        fontWeight: FontWeight.w700)),
                    Text(profile.pinEnabled
                ...
                Switch(value: profile.pinEnabled,
...
// ── PIN Input Dialog Widget — FIXED ────────────────────────
// প্রতিবার নতুন dialog এ fresh empty input
class _PinDialog extends StatefulWidget {
  ...
  const _PinDialog({
    ...
    required this.onComplete, required this.onCancel});
...
class _PinDialogState extends State<_PinDialog> {
  ...
  void _add(String d) {
    ...
    setState(() => _pin += d);
    if (_pin.length == 4) {
      // ০.৩ সেকেন্ড দেখাক তারপর complete
      Future.delayed(const Duration(milliseconds: 300), () {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.center,
            children: ['1','2','3'].map((d) => Padding(
              padding: const EdgeInsets.all(4),
              child: _numBtn(d, () => _add(d)))).toList()),
          const SizedBox(height: 8),
          Row(mainAxisAlignment: MainAxisAlignment.center,
            children: ['4','5','6'].map((d) => Padding(
              padding: const EdgeInsets.all(4),
              child: _numBtn(d, () => _add(d)))).toList()),
          const SizedBox(height: 8),
          Row(mainAxisAlignment: MainAxisAlignment.center,
            children: ['7','8','9'].map((d) => Padding(
              padding: const EdgeInsets.all(4),
              child: _numBtn(d, () => _add(d)))).toList()),
          const SizedBox(height: 8),
          Row(mainAxisAlignment: MainAxisAlignment.center,
